...
function isEnglishEnough(s: string): boolean {
  ...
  // allow some Japanese in quoted proper nouns, but not body
  return jp <= 3 && en >= 20;
}
...
function isJapaneseEnough(s: string): boolean {
  ...
  if (!t) return false;
  const jp = countJapaneseChars(t);
  // Japanese mode should have some Japanese chars
  return jp >= 10;
}
...
SCOPE (STRICT):
- You will be given "REPLY TARGET".
- Respond ONLY to REPLY TARGET.
- Ignore older quoted messages.
...
function languageOutputOK(language: "Japanese" | "English" | "Bilingual", body: string): boolean {
  const t = body.trim();
  if (!t) return false;
...
export async function POST(req: Request) {
  try {
    ...
    const { replyTarget, background } = extractLatestIncomingMessage(requestText);
    ...
    if (!languageOutputOK(language, out.body)) {
      ...
      // If still wrong, fail fast (do NOT attach mismatched signatures)
      if (!languageOutputOK(language, out.body)) {
        return NextResponse.json(
          {
